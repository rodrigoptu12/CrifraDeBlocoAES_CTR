...
def montar_imagem(img, tam, name):
    d = img
    d = bytes(d)
...
def montar_arquivo(texto, nome_do_arquivo):
    with open(nome_do_arquivo, "wb") as arquivo:
        arquivo.write(bytearray(texto))
    return texto
...
if __name__ == '__main__':
    while True:
        ...
        imgName = input("Digite o nome do arquivo(se imagem -> ex: tux.bmp): ")
        nRodadas = int(input("Digite o numero de rodadas: "))
        isImg = input("Digite 1 para imagem e 2 para txt: ")
        key = input(
        ...
        if isImg == '1':
            blocos, resto, size = criar_blocos_bmp(imgName, True)
        else:
            blocos, resto, size = criar_blocos_bmp(imgName, False)
        restoEx = resto + [0x0] * (16 - len(resto))
        adicionado = 16 - len(resto)
        blocos += [restoEx]
        blocos_cifrados = ctr_mode(blocos, sub_chaves, nRodadas)
        blocao = Compressar(blocos_cifrados)
        blocao = blocao[:-adicionado]
        if isImg == '1':
            ImgCifrada = montar_imagem(blocao, size, "result")
        else:
            TextCifrada = montar_arquivo(blocao, "result.txt")
